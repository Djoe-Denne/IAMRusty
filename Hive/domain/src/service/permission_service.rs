...
where
    ...
    MRP: MemberRoleRepository,
{
    pub fn new(organization_service: Arc<OS>, member_service: Arc<MS>, member_role_repo: Arc<MRP>, resource_name: String) -> Self {
    ...
    MRP: MemberRoleRepository + Send + Sync,
{
    async fn fetch_permissions(
        ...
        debug!("getting member with id: {:?}", user_id);
        let member = self
            .member_service
        ...
        let domain_permissions = member_roles
            .iter()
            .filter(|role| role.role_permission.resource.name == self.resource_name)
