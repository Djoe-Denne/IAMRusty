...
use hive_domain::{entity::{permission::{Permission, PermissionLevel}, resource::Resource, RolePermission}};
...
impl RolePermissionMapper {
    pub fn to_domain(model: role_permissions::Model) -> RolePermission {
        RolePermission::new(
            ...
            &Permission::new(PermissionLevel::from_str(model.permission_id.as_str()).unwrap(), None, Some(model.created_at)),
...
impl RolePermissionReadRepository for RolePermissionReadRepositoryImpl {
    ...
    async fn find_by_organization_role(&self, organization_id: &Uuid, resource_type: &str, permission: &str) -> Result<Option<RolePermission>, DomainError> {
        ...
        Ok(role_permissions.into_iter().map(RolePermissionMapper::to_domain).next())
    }
    ...
    async fn find_by_organization_roles(&self, organization_id: &Uuid, role_permissions: &Vec<RolePermission>) -> Result<Vec<RolePermission>, DomainError> {
        ...
        let role_permissions = OrganizationRolePermissions::find()
            ...
            .filter(role_permissions::Column::PermissionId.is_in(role_permissions.iter().map(|role| role.permission.level.to_str().to_string()).collect::<Vec<_>>()))
            .all(self.db.as_ref())
            .await
            .map_err(|e| DomainError::internal_error(&e.to_string()))?;

        Ok(role_permissions.into_iter().map(RolePermissionMapper::to_domain).collect())
    }
...
impl RolePermissionWriteRepository for RolePermissionWriteRepositoryImpl {
    async fn save(&self, organization_id: &Uuid, role_permission: &RolePermission) -> Result<RolePermission, DomainError> {
        ...
        let exists = role_permission.id.is_some() && OrganizationRolePermissions::find_by_id(role_permission.id.unwrap())
            .one(self.db.as_ref())
            .await
            .map_err(|e| DomainError::internal_error(&e.to_string()))?
            .is_some();
