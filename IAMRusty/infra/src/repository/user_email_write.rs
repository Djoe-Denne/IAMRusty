...
use domain::port::repository::UserEmailWriteRepository;
...
impl UserEmailWriteRepository for UserEmailWriteRepositoryImpl {
    ...
    async fn delete(&self, id: Uuid) -> Result<(), Self::Error> {
        ...
        let result = UserEmails::delete_by_id(id)
            .exec(self.db.as_ref())
            .await?;
    ...
    async fn set_as_primary(&self, user_id: Uuid, email_id: Uuid) -> Result<(), Self::Error> {
        ...
        let result = UserEmails::update_many()
            ...
            .col_expr(user_emails::Column::UpdatedAt, Expr::current_timestamp().into())
