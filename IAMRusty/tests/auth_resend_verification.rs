...
async fn test_resend_verification_success() {
    ...
    assert!(message.contains("If your email is registered"), "Should contain generic success message");
    ...
    let count: i64 = verification_count.try_get("", "count").expect("Failed to get token count");
    assert!(count > 0, "New verification token should be created");

    // ✅ Verify email is still unverified (only resend, not auto-verify)
    assert!(user_email.check(db.clone()).await.expect("Failed to check user email"));
    let email_status = db
        .query_one(sea_orm::Statement::from_string(
            sea_orm::DatabaseBackend::Postgres,
...
async fn test_resend_verification_email_already_verified() {
    ...
    assert_eq!(response.status(), 200, "Should return 200 for already verified email to prevent user enumeration");
    ...
    assert!(response_body.get("message").is_some(), "Should contain message");
    let message = response_body["message"].as_str().unwrap();
    // Should contain generic message that doesn't reveal email status
    assert!(message.contains("If your email is registered"), "Should contain generic message that doesn't reveal email status");
    ...
    let count: i64 = verification_count.try_get("", "count").expect("Failed to get token count");
    assert_eq!(count, 0, "No verification token should be created for already verified email");
}
...
async fn test_resend_verification_email_not_found() {
    ...
    assert_eq!(response.status(), 200, "Should return 200 for non-existent email to prevent user enumeration");
...
async fn test_resend_verification_invalid_email_format() {
    ...
    for invalid_email in invalid_emails {
        ...
        assert_eq!(response.status(), 422, "Should return 422 for invalid email format at framework level: {}", invalid_email);
        ...
        assert!(response_body.get("error").is_some(), "Should contain validation error for: {}", invalid_email);
    }
}
...
async fn test_resend_verification_missing_email_field() {
    ...
    assert_eq!(response.status(), 422, "Should return 422 for missing email field at framework level");
    ...
    assert!(response_body.get("error").is_some(), "Should contain validation error");
}
...
async fn test_resend_verification_wrong_content_type() {
    ...
    assert_eq!(response.status(), 422, "Should return 422 for wrong content type at framework level");
...
async fn test_resend_verification_multiple_times_same_email() {
    ...
    let count: i64 = verification_count.try_get("", "count").expect("Failed to get token count");
    assert!(count >= 1, "At least one verification token should exist after multiple resends");
...
async fn test_resend_verification_case_insensitive_email() {
    ...
    for email_variant in email_variations {
        ...
        assert_eq!(response.status(), 200, "Should always return 200 to prevent information leakage for email: {}", email_variant);
        ...
        assert!(response_body.get("message").is_some(), "Should contain message for: {}", email_variant);
        let message = response_body["message"].as_str().unwrap();
        assert!(message.contains("If your email is registered"), "Should contain generic message that doesn't reveal case sensitivity behavior");
    }
}
...
async fn test_resend_verification_database_consistency() {
    ...
    assert!(final_count_val > initial_count_val, 
           "Should have more verification tokens after resend: initial={}, final={}", 
           initial_count_val, final_count_val);
    ...
    let verification_token: String = token_data.try_get("", "verification_token").expect("Failed to get token");
    assert!(!verification_token.is_empty(), "Verification token should not be empty");
