...
impl TestSqs {
    ...
    pub async fn new() -> Result<Self, Box<dyn std::error::Error>> {
        ...
        let endpoint_url = sqs_config.endpoint_url().unwrap_or("http://localhost:4566".to_string());

        let access_key_id = sqs_config.access_key_id.clone().unwrap_or("test".to_string());
        let secret_access_key = sqs_config.secret_access_key.clone().unwrap_or("test".to_string());
    ...
    async fn create_sqs_client(endpoint_url: &str, region: &str) -> Result<Client, Box<dyn std::error::Error>> {
    ...
    async fn wait_for_localstack(endpoint_url: &str) -> Result<(), Box<dyn std::error::Error>> {
        ...
        while attempts < max_attempts {
            ...
            if attempts < max_attempts {
                debug!("Retrying LocalStack connection in 1 second... (attempt {}/{})", attempts, max_attempts);
                tokio::time::sleep(Duration::from_secs(1)).await;
            }
        }
        
        Err(format!("LocalStack failed to become ready after {} attempts", max_attempts).into())
    }
    
    /// Create test queue using the queue name from configuration
    async fn create_test_queue(client: &Client, sqs_config: &SqsConfig) -> Result<String, Box<dyn std::error::Error>> {
    ...
    pub async fn send_message(&self, message_body: &str) -> Result<String, Box<dyn std::error::Error>> {
    ...
    pub async fn receive_messages(&self, max_messages: i32, wait_time_seconds: i32) -> Result<Vec<Message>, Box<dyn std::error::Error>> {
    ...
    pub async fn delete_message(&self, receipt_handle: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    pub async fn get_all_messages(&self, max_wait_secs: u64) -> Result<Vec<String>, Box<dyn std::error::Error>> {
        debug!("Getting all messages from queue with max wait: {} seconds", max_wait_secs);
    ...
    pub async fn wait_for_messages(&self, expected_count: usize, max_wait_secs: u64) -> Result<Vec<String>, Box<dyn std::error::Error>> {
        debug!("Waiting for {} messages with max wait: {} seconds", expected_count, max_wait_secs);
        ...
        if all_messages.len() < expected_count {
            return Err(format!(
                ...
                all_messages.len(), expected_count, max_wait_secs
            ).into());
        }
...
async fn get_or_create_test_sqs_container() -> Result<(Arc<TestSqsContainer>, SqsConfig), Box<dyn std::error::Error>> {
    let container_mutex = TEST_SQS_CONTAINER.get_or_init(|| {
...
impl TestSqsFixture {
    ...
    pub async fn send_test_event(&self, event_type: &str, aggregate_id: &str, data: Value) -> Result<String, Box<dyn std::error::Error>> {
