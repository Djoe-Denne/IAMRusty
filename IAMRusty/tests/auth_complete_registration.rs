...
async fn test_complete_registration_success() {
    ...
    if status != 200 {
        let error_body: Value = completion_response.json().await.expect("Should return JSON response");
        panic!("Expected 200 but got {}. Error response: {}", status, serde_json::to_string_pretty(&error_body).unwrap());
    }
    
    let response_body: Value = completion_response.json().await.expect("Should return JSON response");

    // Verify response structure
    assert_eq!(response_body["user"]["username"].as_str().unwrap(), "completeduser");
    assert_eq!(response_body["user"]["email"].as_str().unwrap(), "completion@example.com");
...
async fn test_complete_registration_invalid_token_signature() {
    ...
    assert_eq!(completion_response.status(), 400, "Should return 400 for invalid token signature");
    ...
    assert_eq!(error_response["error"]["message"].as_str().unwrap(), "Invalid registration token signature");
}
...
async fn test_complete_registration_expired_token() {
    ...
    let error_response: Value = completion_response.json().await.expect("Should return JSON response");
    assert_eq!(error_response["error"]["error_code"].as_str().unwrap(), "token_expired");
    assert_eq!(error_response["error"]["message"].as_str().unwrap(), "Registration token has expired");
}
...
async fn test_complete_registration_valid_username_formats() {
    ...
    for valid_username in valid_usernames {
        ...
        let response_body: Value = completion_response.json().await.expect("Should return JSON response");
        assert_eq!(response_body["user"]["username"].as_str().unwrap(), valid_username);
    }
}
...
async fn test_complete_registration_malformed_request() {
    ...
    assert_eq!(response.status(), 422, "Should return 422 for missing registration_token");
    ...
    assert_eq!(response.status(), 422, "Should return 422 for missing username");
    ...
    assert_eq!(response.status(), 422, "Should return 422 for empty request body");
...
async fn test_complete_registration_updates_user_record() {
    ...
    let username: String = user_record.try_get("", "username").expect("Should have username");
    assert_eq!(username, "updateduser", "Username should be updated in database");
}
...
async fn test_complete_registration_user_can_login_afterward() {
    ...
    assert_eq!(login_response.status(), 200, "Should be able to login after completing registration and verifying email");
    
    let login_body: Value = login_response.json().await.expect("Should return JSON response");
    assert_eq!(login_body["user"]["username"].as_str().unwrap(), "loginuser");
    assert_eq!(login_body["user"]["email"].as_str().unwrap(), "loginafter@example.com");
    assert!(login_body["access_token"].is_string(), "Should return access token");
    assert!(login_body["refresh_token"].is_string(), "Should return refresh token");
} 