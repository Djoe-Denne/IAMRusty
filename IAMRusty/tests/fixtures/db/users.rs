...
use infra::repository::entity::users::{Entity as UsersEntity, Model as UserModel, ActiveModel as UserActiveModel};
...
impl UserFixtureBuilder {
    ...
    pub fn bob(self) -> Self {
        self.username("bob")
            .avatar_url(Some("https://avatars.githubusercontent.com/u/789012".to_string()))
    }
    
    /// Create Alice user (GitLab user)
    pub fn alice(self) -> Self {
        self.username("alice")
            .avatar_url(Some("https://gitlab.com/uploads/-/system/user/avatar/123/avatar.png".to_string()))
    }
...
impl UserFixture {
    ...
    pub async fn check(&self, db: Arc<DatabaseConnection>) -> Result<bool, DbErr> {
        ...
        let db_user = UsersEntity::find_by_id(self.model().id)
            .one(&*db)
            .await?;
