...
use infra::repository::entity::provider_tokens::{Entity as ProviderTokensEntity, Model as ProviderTokenModel, ActiveModel as ProviderTokenActiveModel};
...
impl DbFixture<ProviderTokensEntity, ProviderTokenModel, ProviderTokenActiveModel> for ProviderTokenFixtureBuilder {
    async fn commit(self, db: &DatabaseConnection) -> Result<CommittedFixture<ProviderTokenModel>, DbErr> {
    ...
    fn model(&self) -> ProviderTokenActiveModel {
        ...
        ProviderTokenActiveModel {
            ...
            provider: ActiveValue::Set(self.provider.clone().unwrap_or_else(|| "github".to_string())),
            access_token: ActiveValue::Set(self.access_token.clone().unwrap_or_else(TestData::access_token)),
            refresh_token: ActiveValue::Set(self.refresh_token.clone().unwrap_or(Some(TestData::refresh_token()))),
            ...
            provider_user_id: ActiveValue::Set(self.provider_user_id.clone().unwrap_or_else(TestData::provider_user_id)),
        }
    }
}
