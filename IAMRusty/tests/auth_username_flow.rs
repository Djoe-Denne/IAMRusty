...
async fn test_new_user_signup_returns_201_with_registration_token() {
    ...
    // Verify response
    assert_eq!(response.status(), 201, "Should return 201 for new user signup");
    ...
    assert!(response_body["user"]["id"].is_string(), "Should return user ID");
    assert_eq!(response_body["user"]["email"].as_str().unwrap(), "newuser@example.com");
    assert!(response_body["registration_token"].is_string(), "Should return registration token");
    assert_eq!(response_body["requires_username"].as_bool().unwrap(), true);
    assert!(response_body["message"].is_string(), "Should return message");
    
    // Verify username is not present (incomplete registration)
    assert!(response_body["user"]["username"].is_null(), "Username should not be present for incomplete registration");
}
...
async fn test_registration_token_is_valid_rsa_signed_jwt() {
    ...
    assert!(registration_token.starts_with("eyJ"), "Should be a valid JWT");
...
async fn test_user_record_created_with_null_username_pending_status() {
    ...
    assert!(user_record.try_get::<Option<String>>("", "username").unwrap().is_none(), 
...
async fn test_existing_user_signup_returns_200_with_tokens() {
    ...
    // Verify response
    assert_eq!(response.status(), 200, "Should return 200 for existing user");
    
    let response_body: Value = response.json().await.expect("Should return JSON response");

    // Verify response structure
    assert_eq!(response_body["user"]["id"].as_str().unwrap(), existing_user.id().to_string());
    assert_eq!(response_body["user"]["username"].as_str().unwrap(), existing_user.username().unwrap());
    assert_eq!(response_body["user"]["email"].as_str().unwrap(), primary_email.email());
    assert!(response_body["access_token"].is_string(), "Should return access token");
    assert!(response_body["expires_in"].is_number(), "Should return expires_in");
    assert!(response_body["refresh_token"].is_string(), "Should return refresh token");
    assert!(response_body["message"].is_string(), "Should return message");
}
...
async fn test_password_auth_method_added_to_existing_user() {
    ...
    let user_record = db
        .query_one(sea_orm::Statement::from_string(
            sea_orm::DatabaseBackend::Postgres,
            format!("SELECT password_hash FROM users WHERE id = '{}'", existing_user.id()),
        ))
        .await
        .expect("Failed to query user")
        .expect("User should exist");
    ...
    assert_ne!(password_hash.unwrap(), "newPassword123", "Password should be hashed");
}
...
async fn test_login_completed_user_returns_200_with_tokens() {
    ...
    let password_hash = password_adapter.hash_password("validPassword123").await.expect("Failed to hash password");
    ...
    // Verify response
    assert_eq!(response.status(), 200, "Should return 200 for completed user login");
    
    let response_body: Value = response.json().await.expect("Should return JSON response");

    // Verify response structure
    assert_eq!(response_body["user"]["id"].as_str().unwrap(), completed_user.id().to_string());
    assert_eq!(response_body["user"]["username"].as_str().unwrap(), completed_user.username().unwrap());
    assert_eq!(response_body["user"]["email"].as_str().unwrap(), primary_email.email());
    assert!(response_body["access_token"].is_string(), "Should return access token");
    assert!(response_body["expires_in"].is_number(), "Should return expires_in");
    assert!(response_body["refresh_token"].is_string(), "Should return refresh token");
...
async fn test_login_incomplete_user_returns_423_with_registration_token() {
    ...
    // Verify response
    assert_eq!(response.status(), 423, "Should return 423 for incomplete registration");
    
    let response_body: Value = response.json().await.expect("Should return JSON response");

    // Verify response structure
    assert_eq!(response_body["error"].as_str().unwrap(), "registration_incomplete");
    assert!(response_body["message"].is_string(), "Should return message");
    assert!(response_body["registration_token"].is_string(), "Should return new registration token");
}
...
async fn test_login_invalid_credentials_returns_401() {
    ...
    let password_service = Arc::new(infra::auth::PasswordService::new());
    let password_adapter = infra::auth::PasswordServiceAdapter::new(password_service);
    let password_hash = password_adapter.hash_password("correctPassword123").await.expect("Failed to hash password");
    ...
    // Verify response
    assert_eq!(response.status(), 401, "Should return 401 for invalid credentials");
}
...
async fn test_login_nonexistent_email_returns_401() {
    ...
    // Verify response
    assert_eq!(response.status(), 401, "Should return 401 for non-existent email");
}
...
async fn test_oauth_callback_new_user_returns_202_with_registration_token() {
    ...
    // Verify response
    assert_eq!(callback_response.status(), 202, "Should return 202 for new user OAuth");
    
    let response_body: Value = callback_response.json().await.expect("Should return JSON response");

    // Verify response structure
    assert_eq!(response_body["operation"].as_str().unwrap(), "registration_required");
    assert!(response_body["registration_token"].is_string(), "Should return registration token");
    assert!(response_body["provider_info"]["email"].is_string(), "Should return provider email");
    assert!(response_body["provider_info"]["suggested_username"].is_string(), "Should suggest username");
    assert_eq!(response_body["requires_username"].as_bool().unwrap(), true);
}
...
async fn test_registration_token_contains_oauth_provider_info() {
    ...
    assert!(provider_info["email"].is_string(), "Should contain provider email");
    assert!(provider_info["avatar"].is_string(), "Should contain provider avatar");
    assert!(provider_info["suggested_username"].is_string(), "Should suggest username based on provider data");
    ...
    assert!(payload["provider_info"].is_object(), "JWT should contain provider info");
}
...
async fn test_complete_registration_valid_token_available_username_returns_200() {
    ...
    // Verify response
    assert_eq!(completion_response.status(), 200, "Should return 200 for successful registration completion");
    
    let response_body: Value = completion_response.json().await.expect("Should return JSON response");

    // Verify response structure
    assert_eq!(response_body["user"]["username"].as_str().unwrap(), "uniqueuser123");
    assert_eq!(response_body["user"]["email"].as_str().unwrap(), "registration@example.com");
    assert!(response_body["access_token"].is_string(), "Should return access token");
    assert!(response_body["expires_in"].is_number(), "Should return expires_in");
    assert!(response_body["refresh_token"].is_string(), "Should return refresh token");
}
...
async fn test_complete_registration_invalidates_token_after_use() {
    ...
    assert_eq!(second_completion.status(), 400, "Should return 400 for reused token");
    
    let error_response: Value = second_completion.json().await.expect("Should return JSON response");
    assert_eq!(error_response["error"]["error_code"].as_str().unwrap(), "invalid_token");
}
...
async fn test_complete_registration_taken_username_returns_409() {
    ...
    // Verify response
    assert_eq!(completion_response.status(), 409, "Should return 409 for taken username");
}
...
async fn test_complete_registration_invalid_username_format_returns_422() {
    ...
    for invalid_username in invalid_usernames {
        ...
        assert_eq!(completion_response.status(), 422, 
...
async fn test_username_check_taken_username_returns_false_with_suggestions() {
    ...
    for suggestion in suggestions {
        ...
        assert!(suggestion_str.starts_with("johndoe"), "Suggestions should be based on original username");
...
async fn test_complete_email_first_flow() {
    ...
    let completion_body: Value = completion_response.json().await.expect("Should return JSON response");
    ...
    assert_eq!(login_response.status(), 401, "Login should fail before email verification");
    ...
    assert_eq!(login_response.status(), 200, "Login should succeed after email verification");
...
async fn test_complete_oauth_first_flow() {
    ...
    let callback_body: Value = callback_response.json().await.expect("Should return JSON response");
