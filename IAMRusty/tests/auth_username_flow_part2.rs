...
async fn test_oauth_provider_already_linked_to_same_user() {
    ...
    assert!(
        oauth_start_response.status() == 303 || oauth_start_response.status() == 409,
...
async fn test_oauth_provider_linked_to_different_user_returns_409() {
    ...
    if oauth_start_response.status() == 303 {
        ...
        let error_response: Value = callback_response
            .json()
            .await
...
async fn test_registration_token_contains_required_claims() {
    ...
    assert!(
        exp <= now + twenty_four_hours + 60, // +60 for clock skew
...
async fn test_same_email_signup_after_incomplete_registration() {
    ...
    assert_ne!(
        ...
        "Should generate new registration token on retry"
    );
}
...
async fn test_no_duplicate_user_records_created_on_retry() {
    ...
    let user_count = db
        .query_one(sea_orm::Statement::from_string(
            ...
            "SELECT COUNT(*) as count FROM user_emails WHERE email = 'duplicatetest@example.com'"
                .to_string(),
        ))
        .await
    ...
    let count: i64 = user_count
        .try_get("", "count")
        .expect("Failed to get count");
    assert_eq!(
        count, 1,
        "Should only create one user record despite multiple signups"
    );
}
...
async fn test_user_id_remains_consistent_across_retries() {
    ...
    assert_eq!(
        ...
        "User ID should remain consistent across retries"
    );
}
...
async fn test_no_sensitive_data_exposed_in_error_messages() {
    ...
    assert!(
        !error_message.to_lowercase().contains("user not found"),
        "Error message should not reveal user existence"
    );
    assert!(
        !error_message.to_lowercase().contains("email not found"),
...
async fn test_proper_https_enforcement_for_token_transmission() {
    ...
    assert!(
        ...
        "Test server should be accessible"
    );
