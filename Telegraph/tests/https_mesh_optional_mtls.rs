//! T14b — Telegraph dual-bind HTTP+HTTPS with optional mesh client CA (no docker).
...
use telegraph_http_server::{create_prefixed_router, SERVICE_PREFIX};
...
async fn spawn_server(config: ServerConfig) -> tokio::task::JoinHandle<anyhow::Result<()>> {
    let router = create_prefixed_router(app_state(), Arc::new(ReadinessProbe::new("telegraph")));
