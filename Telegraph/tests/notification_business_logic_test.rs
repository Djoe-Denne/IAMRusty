...
async fn test_mark_as_read_operation() {
    ...
    assert!(final_notification.is_read, "Notification should be marked as read");
    assert!(final_notification.read_at.is_some(), "read_at should be set");
}
...
async fn test_large_dataset_pagination() {
    ...
    for per_page in page_sizes {
        ...
        loop {
            let response = client.get(format!("{}/api/notifications?page={}&per_page={}", base_url, page, per_page))
                .header(header::AUTHORIZATION, format!("Bearer {}", jwt_token))
                .send()
                .await
                .expect("Failed to send request");

            assert_eq!(response.status(), StatusCode::OK, "Page {} should be successful", page);

            let result: Value = response.json().await.expect("Failed to parse response");

            let notifications = result["notifications"].as_array().expect("Notifications should be an array");
            let has_more = result["has_more"].as_bool().expect("has_more should be a boolean");

            // Collect notification IDs to ensure no duplicates
            for notification in notifications {
                let id = notification["id"].as_str().expect("id should be a string");
                assert!(all_notification_ids.insert(id.to_string()), 
        ...
        assert_eq!(total_collected, 50, 
                  "Should collect exactly 50 notifications with per_page={}", per_page);
        assert_eq!(all_notification_ids.len(), 50, 
...
async fn test_expired_notifications_handling() {
    ...
    assert_eq!(notifications.len(), 2, "Should return both active and expired notifications");
    
    // Find expired notification and verify it's marked or handled appropriately
    let expired_notification = notifications.iter()
...
async fn test_multi_user_isolation_comprehensive() {
    ...
    let response_a = client.get(format!("{}/api/notifications", base_url))
    ...
    let notifications_a = result_a["notifications"].as_array().expect("Notifications should be an array");
    assert_eq!(notifications_a.len(), 1, "User A should see only 1 notification");
    assert_eq!(notifications_a[0]["title"].as_str().expect("title should be a string"), "User A notification");

    // User B should only see their notification  
    let response_b = client.get(format!("{}/api/notifications", base_url))
    ...
    let notifications_b = result_b["notifications"].as_array().expect("Notifications should be an array");
    assert_eq!(notifications_b.len(), 1, "User B should see only 1 notification");
    assert_eq!(notifications_b[0]["title"].as_str().expect("title should be a string"), "User B notification");

    // Test cross-user access attempt - User A tries to mark User B's notification as read
    let cross_access_response = client.put(format!("{}/api/notifications/{}/read", base_url, notification_b.id()))
        .header(header::AUTHORIZATION, format!("Bearer {}", jwt_a))
        .send()
        .await
        .expect("Failed to send request");

    assert_eq!(cross_access_response.status(), StatusCode::FORBIDDEN, 
    ...
    assert!(!verification_notification.is_read, "User B's notification should remain unread after unauthorized access attempt");
}
...
async fn test_complex_notification_filtering() {
    ...
    let all_response = client.get(format!("{}/api/notifications", base_url))
        .header(header::AUTHORIZATION, format!("Bearer {}", jwt_token))
        .send()
        .await
        .expect("Failed to send request");
    ...
    assert_eq!(all_result["total_count"].as_u64().unwrap(), 5, "Should have 5 total notifications");

    // Test 2: Get only unread notifications
    let unread_response = client.get(format!("{}/api/notifications?unread_only=true", base_url))
        .header(header::AUTHORIZATION, format!("Bearer {}", jwt_token))
        .send()
        .await
        .expect("Failed to send request");
    ...
    let unread_result: Value = unread_response.json().await.expect("Failed to parse response");
    
    let unread_notifications = unread_result["notifications"].as_array().expect("Notifications should be an array");
    assert_eq!(unread_notifications.len(), 3, "Should have 3 unread notifications");
    ...
    assert_eq!(count_result["unread_count"].as_u64().unwrap(), 3, 
