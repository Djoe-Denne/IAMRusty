    ...
    pub fn new(config: TemplateConfig) -> Result<Self, DomainError> {
        ...
        if !template_dir.exists() {
            std::fs::create_dir_all(&template_dir).map_err(|e| {
                DomainError::template_load_error(format!(
                    "Failed to create template directory '{}': {}",
                    config.template_dir, e
                ))
            })?;
            info!(
...
impl TemplateService for TeraTemplateService {
    ...
    async fn render_template(
        ...
        debug!(
            ...
            template_name, merged_variables
        );
        ...
        match mode {
            CommunicationMode::Email => {
                ...
                let text_body = self
                    .render_tera_template(&text_template, &merged_variables)
                    .await?;
                ...
                let html_body = if let Some(html_template_name) = html_template {
                    match self
                        .render_tera_template(&html_template_name, &merged_variables)
                        .await
                    {
                        Ok(html) => Some(html),
                        Err(e) => {
