...
pub trait NotificationWriteRepository: Send + Sync {
    ...
    async fn create_notification(&self, notification: NotificationCommunication) -> Result<NotificationCommunication, DomainError>;
    ...
    async fn create_delivery(&self, delivery: MessageDelivery) -> Result<MessageDelivery, DomainError>;

    /// Update a delivery record
    async fn update_delivery_status(&self, delivery_id: Uuid, status: String, error_message: Option<String>) -> Result<MessageDelivery, DomainError>;

    /// Increment delivery attempt
    async fn increment_delivery_attempt(&self, delivery_id: Uuid) -> Result<MessageDelivery, DomainError>;
}
