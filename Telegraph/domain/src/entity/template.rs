...
impl MessageTemplate {
    ...
    pub fn render(
        ...
        match &self.content {
            TemplateContent::Email {
                ...
                html_body: html_body
                    .as_ref()
                    .map(|body| Self::replace_placeholders(body, &merged_variables))
                    .transpose()?,
            ...
            TemplateContent::Sms { text } => Ok(RenderedTemplate::Sms {
    ...
    fn replace_placeholders(
        ...
        if result.contains("{{") && result.contains("}}") {
            return Err(DomainError::invalid_message(
                "Template contains unreplaced placeholders".to_string(),
            ));
        }
    ...
    /// Validate that the mode matches the content
    fn validate_mode_content_match(
        mode: &CommunicationMode,
        content: &TemplateContent,
    ) -> Result<(), DomainError> {
        match (mode, content) {
