...
summary: Hive routes organization-management behavior through a RustyCog command registry whose use cases also publish hive-events for downstream consumers.
provenance:
  extracted: 0.79
  inferred: 0.12
  ambiguous: 0.09
...
- The registry is broader than the live route table: invitation and member-update operations exist as commands even when they are not all registered as live HTTP routes.
...
- [[references/hive-http-api-and-openapi-drift]] - Route-level consumers of the command layer.