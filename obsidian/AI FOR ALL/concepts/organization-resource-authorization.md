...
`[[projects/hive/hive]]` uses Casbin-style route policies, but the real authorization work happens in domain-backed permission fetchers. That gives Hive a richer organization/member/resource model than a static route ACL, while still fitting inside `rustycog_http::RouteBuilder`.
...
- [[references/hive-http-api-and-openapi-drift]] - Live route surface that consumes the permission model.
- [[references/hive-data-model-and-schema]] - Tables that back the role and resource model.
- [[skills/building-organization-management-services]] - Practical workflow that applies this pattern.