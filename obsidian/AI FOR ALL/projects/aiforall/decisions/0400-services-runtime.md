...
summary: "IAM = IdP ; Manifesto ACL/CAS + ticker P2 Implemented. Lazaret P3 Partial. Dual runtime. Factory/host hors livré (0406)."
...
Standalones **et** `oodhive-monolith` (préfixes identiques). Le monolithe compose les routeurs, **n’appelle pas** `run()`, **ne remplace pas** les microservices. `sentinel-sync` hors dual. Lazaret s’y nide sous `/lazaret`. Voir [[projects/aiforall/references/modular-monolith-runtime]] et [[projects/aiforall/skills/running-aiforall-runtime-modes]].
...
`apparatus-contracts` + KV de référence = P0 livré. Factory, host, gateway **K8s** = **hors livré**. Le ticker P2 in-process Manifesto est **Implemented** (ADR-0006). La frontière HTTP de capacités est le BC [[projects/lazaret/lazaret]] (ADR-0007 **Partial**) — ce n’est pas la Factory ni un adapter Kubernetes. Le canon 0406 dit encore « contrôleur / workers / gateway » non livrés : **périmé** pour le ticker in-process et le service Lazaret, vrai pour K8s/Factory/host. ^[ambiguous] Voir [[projects/manifesto/concepts/apparatus-p2-reconciliation]] et [[projects/manifesto/decisions/0007-apparatus-p3-lazaret]].
