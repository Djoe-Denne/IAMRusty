...
- The test is a Hive / Telegraph / Manifesto HTTP IT arranging OpenFGA **Check**. Use `TestOpenFga` (`has_openfga() == true`). `OpenFgaMockService` is only for crate-level fakes inside `rustycog-testing`.
