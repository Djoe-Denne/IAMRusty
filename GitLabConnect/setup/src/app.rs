//! Composition root: empty command registry, deny-all checker, GitLab HMAC router.

use std::sync::Arc;

use anyhow::Error;
use axum::Router;
use gitlab_connect_configuration::AppConfig;
use gitlab_connect_http::{create_app_routes, create_prefixed_router, create_router};
use gitlab_connect_infra::GitLabConnectClient;
...
impl Application {
    /// Wire empty registry, JWT extractor, deny-all checker, GitLab vendor client.
    ///
    /// # Errors
    ///
    /// Returns an error if `hmac_secret` is empty, whitespace-only, shorter
    /// than 16 bytes after trim, auth config is invalid, or GitLab vendor
    /// URLs cannot be parsed.
    pub fn new(config: AppConfig) -> Result<Self, Error> {
        tracing::info!("Initializing GitLab Connect application...");

        let hmac_secret = config.gitlab.hmac_secret.trim();
        ...
        tracing::info!("GitLab Connect application initialized successfully");
    ...
    pub async fn run(self, server_config: ServerConfig) -> Result<(), Error> {
        tracing::info!("Starting GitLab Connect HTTP server...");
