...
use common::{boot_app, test_config, GitLabFixtures, ALLOWED_REDIRECT, TEST_HMAC_SECRET};
use gitlab_connect_http::SERVICE_PREFIX;
...
const TOKEN_ROUTE: &str = "/gitlab-connect/v1/token";
const PROFILE_ROUTE: &str = "/gitlab-connect/v1/profile";
...
async fn missing_hmac_headers_are_unauthorized() {
    ...
    let app = boot_app(&gitlab.base_url()).expect("app");
...
async fn valid_hmac_exchanges_token_and_profile() {
    ...
    gitlab
        ...
        .await
        .setup_successful_user_profile_alice()
        .await;
    let app = boot_app(&gitlab.base_url()).expect("app");
    let server = TestServer::new(app.prefixed_router()).expect("test server");
    ...
    assert_eq!(tokens.access_token, "glpat-test_access_token_12345");
...
fn fifteen_byte_hmac_secret_fails_closed_at_boot() {
    let mut config = test_config("http://127.0.0.1:9");
    config.gitlab.hmac_secret = "123456789012345".to_owned();
    assert!(Application::new(config).is_err());
}
