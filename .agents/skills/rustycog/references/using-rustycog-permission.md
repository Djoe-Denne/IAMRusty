...
- `with_permission_on` binds the **deepest** UUID path segment into the `ResourceRef`. For nested routes whose last UUID is not that object (e.g. `/api/projects/{project_id}/members/{user_id}` typed as `"project"`), call `.with_permission_on_param(Permission::X, "project", "project_id")` instead. Missing or non-UUID named params fail closed with 403.
