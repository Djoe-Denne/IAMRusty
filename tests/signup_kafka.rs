...
async fn test_signup_kafka_integration() {
    ...
    let all_messages = kafka_fixture.kafka.get_all_messages(3).await
    ...
    if all_messages.is_empty() {
        ...
        let retry_messages = kafka_fixture.kafka.get_all_messages(5).await
    ...
    let messages = if all_messages.is_empty() {
        kafka_fixture.kafka.get_all_messages(2).await.expect("Should get messages")
    } else {
        all_messages
    };
    
    // Should have at least one message
    assert!(!messages.is_empty(), "Should have at least one event published to Kafka");
            ...
            if let Some(event_type) = event.get("event_type") {
                if event_type == "UserSignedUp" {
                    ...
                    if let Some(email_verified) = event.get("email_verified") {
                        assert_eq!(email_verified.as_bool().unwrap(), false, "Email should not be verified initially");
                    }
    ...
    assert!(found_signup_event, "Should find a UserSignedUp event in Kafka topic");
    
    println!("✅ Kafka integration test completed successfully");
    println!("   - Test server started with Kafka enabled");
    println!("   - Kafka container running on: {}", kafka_fixture.kafka.brokers);
