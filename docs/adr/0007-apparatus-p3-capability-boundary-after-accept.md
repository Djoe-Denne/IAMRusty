...
Identité = `project_components.id`. **Pas** de second UUID public. Style existant : table `apparatus_bindings`, ids internes BIGSERIAL, `component_id` UUID, `down` réversible. Split par bounded context. **Aucune URL inventée.** Accept **autorise** la migration additive T2 (noms ci-dessous). (état à l’Accept, 2026-09-13) Réalité Unimplemented jusqu’aux preuves.
...
T1 de **caractérisation** (absence `invoke` runtime / `gateway` Manifesto) **n’est pas** une preuve de mécanisme P3. T2–T10 (TDD) sont **autorisés** par cet Accept. Réalité **Implemented** (T1–T10 + T11b + T12 + T13 + T14b livrés ; A-DEC 2026-09-20). Hors-jalon (ne bloquent pas Implemented) : `APP-05` ouvert, 0006 G/E Manifesto, pas K8s, pas de second protocole. Hole mTLS Hive-IAM-Telegraph **fermé** (T14b : HTTPS compose + client CA optionnelle, pas un rustls required production). Hole OpenBao produit hors compose **fermé** (T12). Hole persistance clé CA **fermé** (T13). Hole compose TLS Lazaret **fermé** (T13). `kv_purge` est branché sur `component_removed` / file dédiée. Enrollment T3 in-memory **fermé** (T10). Hole mTLS `/session` **fermé** (T11b).
...
- Accept 2026-09-13 : T2–T7 **autorisés** (TDD). T8 (P3-close) livré. Réalité **Implemented**. T1 (absence) n’est pas une preuve de mécanisme P3.
...
- T9 livré : `Application::prefixed_router` → `create_prefixed_router` ; IT `POST /lazaret/invoke` 2xx ; `POST /invoke` sur routeur préfixé → 404 ; `INVOKE_PATH` reste `/invoke` ; `Application::router()` reste non préfixé. Réalité **Implemented**.
...
- T11b livré : trou mTLS `/session` **fermé** (HTTPS live via `serve_router` + authentification client optionnelle rustycog ; mapping `PeerClientCertificate` → `VerifiedClientCertificate` sans écraser T3/T10 `extensions_mut`). Preuve : `Lazaret/tests/apparatus_p3_t11b_session_mtls.rs`. Réalité **Implemented**.
- T12 livré : trou OpenBao **produit** hors compose **fermé** (`openbao/openbao:2.6.2` pin compose + testcontainer service-local `lazaret_test-openbao` ; KV v2 `secret/` ; `-dev` écoute `0.0.0.0:8200`). T6 IT reste wiremock. Preuve : `Lazaret/tests/apparatus_p3_t12_openbao.rs`. Réalité **Implemented**.
- T13 livré : trou persistance clé CA **fermé** ; trou compose TLS **Lazaret** **fermé** (HTTPS `tls_port` 8080, volume `./Lazaret/certs:/app/certs`, HEALTHCHECK `curl -fk https://localhost:8080/lazaret/health` ; generate-if-absent au boot dans `Application::new`, sans openssl dans l’entrypoint). Défaut d’implémentation CA TTL `24 * 365` heures (8760) — **non figé** par Accept. Preuve : `Lazaret/tests/apparatus_p3_t13_ca_persist.rs`. Réalité **Implemented**.
- T14b livré : trou mTLS Hive-IAM-Telegraph **fermé** comme HTTPS compose + client CA optionnelle (pas un rustls required production) ; dual-bind rustycog `port` 8080 + `tls_port` 8443 ; mesh CA generate-if-absent `./certs/platform-mesh` (distincte de la CA Lazaret). Preuve : `Hive/tests/https_mesh_optional_mtls.rs`, `IAMRusty/tests/https_mesh_optional_mtls.rs`, `Telegraph/tests/https_mesh_optional_mtls.rs`. Réalité **Implemented**.
- Hors-jalon (A-DEC 2026-09-20 : ne bloquent pas Implemented) : `APP-05` ouvert ; 0006 G/E Manifesto ; pas K8s ; pas de second protocole.
...
| Harness P0 comme runtime ou KV de production | ADR-0003, 0004 (harness ≠ prod) |
...
- `APP-01` … `APP-07` (`APP-05` **reste ouvert** — ne pas préempter, point 13). Les options A/B vivent dans ADR-0004 « Non décidé ici ».
