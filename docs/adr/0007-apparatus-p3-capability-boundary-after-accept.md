...
T1 de **caractérisation** (absence `invoke` runtime / `gateway` Manifesto) **n’est pas** une preuve de mécanisme P3. T2–T10 (TDD) sont **autorisés** par cet Accept. Réalité **Partial** (T1–T10 + T11b + T12 + T13 + T14b livrés) ; **pas** Implemented (holes : `APP-05` ouvert, 0006 G/E Manifesto, pas K8s, pas de second protocole). Hole mTLS Hive-IAM-Telegraph **fermé** (T14b : HTTPS compose + client CA optionnelle, pas un rustls required production). Hole OpenBao produit hors compose **fermé** (T12). Hole persistance clé CA **fermé** (T13). Hole compose TLS Lazaret **fermé** (T13). `kv_purge` est branché sur `component_removed` / file dédiée. Enrollment T3 in-memory **fermé** (T10). Hole mTLS `/session` **fermé** (T11b).
