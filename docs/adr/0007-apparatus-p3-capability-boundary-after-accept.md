...
T1 de **caractérisation** (absence `invoke` runtime / `gateway` Manifesto) **n’est pas** une preuve de mécanisme P3. T2–T10 (TDD) sont **autorisés** par cet Accept. Réalité **Partial** (T1–T10 livrés) ; **pas** Implemented (holes : mTLS rustls complete, OpenBao produit hors compose, `APP-05` ouvert, 0006 G/E Manifesto, pas K8s, pas de second protocole). `kv_purge` est branché sur `component_removed` / file dédiée. Enrollment T3 in-memory **fermé** (T10).
...
- Holes (bloquent Implemented, pas T5–T10) : rustls mTLS complete résiduel ; OpenBao produit hors compose ; `APP-05` ouvert ; 0006 G/E Manifesto ; pas K8s ; pas de second protocole.
