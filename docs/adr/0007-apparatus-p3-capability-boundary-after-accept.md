...
T1 de **caractérisation** (absence `invoke` runtime / `gateway` Manifesto) **n’est pas** une preuve de mécanisme P3. T2–T8 (TDD) sont **autorisés** par cet Accept. Réalité **Partial** (T1–T8 livrés) ; **pas** Implemented (holes : mTLS rustls complete / enrollment T3 in-memory, OpenBao produit hors compose, invoke IT vs nest `/lazaret`, `APP-05` ouvert, 0006 G/E Manifesto). `kv_purge` est branché sur `component_removed` / file dédiée.
...
- Holes (bloquent Implemented, pas T5–T8) : rustls mTLS complete résiduel / enrollment T3 in-memory ; OpenBao produit hors compose ; invoke IT `/invoke` vs nest prod `/lazaret` ; `APP-05` ouvert ; 0006 G/E Manifesto ; pas K8s ; pas de second protocole.
