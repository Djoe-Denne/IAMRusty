...
T1 de **caractérisation** (absence `invoke` runtime / `gateway` Manifesto) **n’est pas** une preuve de mécanisme P3. T2–T7 (TDD) sont **autorisés** par cet Accept. Réalité **Partial** (T1–T7 livrés) ; **pas** Implemented (holes : mTLS rustls complete / enrollment T3 in-memory, OpenBao produit hors compose, `kv_purge` non branché sur unbind, invoke IT vs nest `/lazaret`, `APP-05` ouvert, 0006 G/E Manifesto).
...
- Accept 2026-09-13 : T2–T7 **autorisés** (TDD). Réalité **Partial**. T1 (absence) n’est pas une preuve de mécanisme P3.
...
- Holes (bloquent Implemented, pas T5–T7) : rustls mTLS complete résiduel / enrollment T3 in-memory ; OpenBao produit hors compose ; `kv_purge` isolé non branché unbind Manifesto ; invoke IT `/invoke` vs nest prod `/lazaret` ; `APP-05` ouvert ; 0006 G/E Manifesto ; pas K8s ; pas de second protocole.
