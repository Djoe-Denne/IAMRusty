...
T1 de **caractérisation** (absence `invoke` runtime / `gateway` Manifesto) **n’est pas** une preuve de mécanisme P3. T2–T10 (TDD) sont **autorisés** par cet Accept. Réalité **Partial** (T1–T10 + T11b livrés) ; **pas** Implemented (holes : OpenBao produit hors compose, `APP-05` ouvert, 0006 G/E Manifesto, pas K8s, pas de second protocole, compose TLS / mTLS Hive-IAM-Telegraph / persistance clé CA). `kv_purge` est branché sur `component_removed` / file dédiée. Enrollment T3 in-memory **fermé** (T10). Hole mTLS `/session` **fermé** (T11b).
...
- Holes (bloquent Implemented, pas T5–T10) : OpenBao produit hors compose ; `APP-05` ouvert ; 0006 G/E Manifesto ; pas K8s ; pas de second protocole ; compose TLS / mTLS Hive-IAM-Telegraph / persistance de la clé CA (pas un rustls mTLS production complet).
