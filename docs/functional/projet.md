...
- `POST /api/projects/{id}/join` — JWT, public **actif**, `MemberSource::Direct`, grant `read` / `project`, 409 si déjà membre. `POST /members` reste Admin-only.
...
| L-PUBLIC-PARTICIPATE | **Fait** : join = membership `read`. |
